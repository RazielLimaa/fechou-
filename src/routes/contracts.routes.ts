...
const addClauseSchema = z.object({ clause_id: z.coerce.number().int().positive() });
const updateClauseSchema = z.object({ custom_content: z.string().trim().min(1).max(30000) });
const reorderSchema = z.object({
  startIndex: z.coerce.number().int().min(0),
  endIndex: z.coerce.number().int().min(0)
});
const layoutSchema = z.object({ layout_config: z.record(z.unknown()) });
...
router.post('/:id/clauses', async (req: AuthenticatedRequest, res: Response) => {
  ...
  const row = await clauseService.addClauseToContract(parsedId.data, parsed.data.clause_id);
  if (!row) return res.status(404).json({ message: 'Contrato ou cláusula não encontrado.' });
...
router.delete('/:id/clauses/:clauseId', async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (!parsedId.success || !parsedClauseId.success) return res.status(400).json({ message: 'IDs inválidos.' });

  const removed = await clauseService.removeClauseFromContract(parsedId.data, parsedClauseId.data);
...
router.patch('/:id/clauses/:clauseId', async (req: AuthenticatedRequest, res: Response) => {
  ...
  const updated = await clauseService.updateClauseContent(parsedId.data, parsedClauseId.data, parsedBody.data.custom_content);
...
router.patch('/:id/clauses/reorder', async (req: AuthenticatedRequest, res: Response) => {
  ...
  const reordered = await clauseService.reorderClauses(parsedId.data, parsedBody.data.startIndex, parsedBody.data.endIndex);
...
router.patch('/:id/layout', async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (plan !== 'premium' && Object.keys(parsedBody.data.layout_config).length > 0) {
    return res.status(403).json({ message: 'Personalização de layout completa disponível apenas no plano premium.' });
  }

  const updated = await contractService.updateContractLayout(parsedId.data, userId, parsedBody.data.layout_config);
...
router.post('/render', async (req: AuthenticatedRequest, res: Response) => {
  ...
  const rendered = await contractRenderService.renderContract(parsedBody.data.contractId, userId);
...
router.post('/:id/pdf', async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (!pdfBuffer) return res.status(404).json({ message: 'Contrato não encontrado.' });
